...
type AuthResponseBody = AuthApiResponse | ErrorApiResponse | null;
type ErrorResponseBody = ErrorApiResponse | null;
type KekMigrationStatusResponseBody = KekMigrationStatusResponse | ErrorApiResponse | null;
type PasswordSaltResponseBody = PasswordSaltResponse | ErrorApiResponse | null;
...
export async function fetchPasswordSalt(request: BaseAuthApiRequest) {
  ...
  const responseBody = (await response.json().catch(() => null)) as PasswordSaltResponseBody;
...
export async function rotatePasswordRequest(request: RotatePasswordApiRequest) {
  ...
  const responseBody = (await response.json().catch(() => null)) as AuthResponseBody;
...
export async function fetchKekMigrationStatus(request: AuthenticatedApiRequest) {
  ...
  const responseBody = (await response.json().catch(() => null)) as KekMigrationStatusResponseBody;
...
export async function fetchLinkedPrincipals(request: AuthenticatedApiRequest) {
  ...
  const responseBody = (await response.json().catch(() => null)) as LinkedPrincipal[] | ErrorApiResponse | null;
...
export async function fetchApiUsers(request: AuthenticatedApiRequest) {
  ...
  const responseBody = (await response.json().catch(() => null)) as ApiUserResponse[] | ErrorApiResponse | null;
...
export async function fetchApiUser(request: AuthenticatedApiRequest & { apiUserId: string }) {
...
export async function deleteApiUserRequest(request: AuthenticatedApiRequest & { apiUserId: string }) {
  const response = await fetch(
    buildApiUrl(request.baseUrl, `/api/auth/api-users/${encodeURIComponent(request.apiUserId)}`),
    {
      method: 'DELETE',
  ...
  const responseBody = (await response.json().catch(() => null)) as ErrorResponseBody;
...
function isEncryptedBlobPayload(value: unknown): value is EncryptedBlobPayload {
  return !!value &&
    ...
    'nonceHex' in value &&
    'version' in value &&
    value.algorithm === 'xsalsa20-poly1305' &&
...
function isWrappedDekPayload(value: unknown): value is WrappedDekPayload {
  return !!value &&
    ...
    value.algorithm === 'ml-kem-768-encapsulated+xsalsa20-poly1305' &&
